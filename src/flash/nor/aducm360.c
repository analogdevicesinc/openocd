/***************************************************************************
 *   Copyright (C) 2015 Analog Devices, by Ivan Buliev                                    *
 *                                                                         *
...
{
	LOG_DEBUG("aducm360_flash_bank_command executed.\n");
...
{
	LOG_DEBUG("aducm360_protect_check not implemented yet.\n");
	return ERROR_OK;
}
...
static int aducm360_mass_erase(struct target *target)
{
	...
	target_read_u32(target, ADUCM360_FLASH_BASE + ADUCM360_FLASH_FEESTA,
			&value);
...
static int aducm360_page_erase(struct target *target, uint32_t padd)
{
	...
	LOG_DEBUG("performing sector erase (at 0x%08lX)... ",
		  (unsigned long)padd);
	...
	target_write_u32(target, ADUCM360_FLASH_BASE + ADUCM360_FLASH_FEEADR0L,
			 padd & 0xFFFF);
	target_write_u32(target, ADUCM360_FLASH_BASE + ADUCM360_FLASH_FEEADR0H,
			 (padd >> 16) & 0xFFFF);
...
static int aducm360_erase(struct flash_bank *bank, int first, int last)
{
	...
	LOG_DEBUG("performing aducm360_erase(%d,%d)...\n", first, last);

	if (((first | last) == 0)
	    || ((first == 0) && (last >= bank->num_sectors))) {
...
/* ----------------------------------------------------------------------- */
static int aducm360_protect(struct flash_bank *bank, int set, int first,
			    int last)
{
	LOG_DEBUG("aducm360_protect not implemented yet.\n");
	return ERROR_FLASH_OPERATION_FAILED;
}

/* ----------------------------------------------------------------------- */
static int aducm360_write_block_sync(struct flash_bank *bank,
				     ...
				     uint32_t offset, uint32_t count)
{
	...
	LOG_DEBUG("'aducm360_write_block_sync' requested, dst:0x%08lX, count:0x%08lX(%d) bytes.\n",
	...
	LOG_DEBUG("allocating space for the aducm360_helper_sync code...\n");
	...
	LOG_DEBUG("allocated at 0x%08lX .. 0x%08lX ...\n",
	...
	LOG_DEBUG("sending the aducm360_write_block code to the target...\n");
	...
	LOG_DEBUG("Helper's RAM start address:0x%08lX, exit point address:0x%08lX\n",
	...
	LOG_DEBUG("buffer space of 0x%08lX bytes was successfully allocated.\n",
		  (unsigned long)target_buffer_size);
	...
	while (count > 0) {
		uint32_t thisrun_count =
		    (count > target_buffer_size) ? target_buffer_size : count;

		LOG_DEBUG
		    ("Copying chunk %02d: ROM:0x%08lX-0x%08lX, bytes:%d(0x%08lX)\n",
		     ++ix, (unsigned long)address,
		     (unsigned long)address + (unsigned long)thisrun_count - 1,
		     thisrun_count, (unsigned long)thisrun_count);

		// ----- Upload the chunk -----
		retval =
		    target_write_buffer(target, target_buffer->address,
		...
		LOG_DEBUG("Programming chunk %02d...\n", ix);
		retval = target_run_algorithm(target, 0, NULL, 5,
					      reg_params, entry_point,
					      exit_point, 10000, &armv7m_algo);

		LOG_DEBUG("retval = 0x%08X...\n", (unsigned)retval);
		if (retval != ERROR_OK) {
			LOG_ERROR
...
/* ----------------------------------------------------------------------- */
static int aducm360_write_block_async(struct flash_bank *bank,
				      const uint8_t * buffer,
				      uint32_t offset, uint32_t count)
{
	struct target *target = bank->target;
	...
	LOG_DEBUG("'aducm360_write_block_async' requested, dst:0x%08lX, count:0x%08lX(%d) bytes.\n",
	     (unsigned long)address, (unsigned long)count, count);
	...
	target_buffer_set_u32_array(target, code,
				    ...
				    aducm360_flash_write_code);
	retval =
	    target_write_buffer(target, helper->address, sizeof(code), code);
	if (retval != ERROR_OK)
		return retval;
	entry_point = helper->address;
	LOG_DEBUG("Helper's RAM start address:0x%08lX, exit point address:0x%08lX\n",
	...
	if (retval != ERROR_OK) {
		...
		if (res) {
			LOG_ERROR
			    ("aducm360 fast async algorithm reports an error (%02X)",
			     res);
			retval = ERROR_FAIL;
		}
	}
				   ...
				   uint32_t offset, uint32_t count)
{
	...
	LOG_DEBUG("performing slow write (offset=0x%08lX, count=0x%08lX)... \n",
			  ...
			  uint32_t offset, uint32_t count)
{
	...
	if (retval != ERROR_OK) {
		if (retval == ERROR_TARGET_RESOURCE_NOT_AVAILABLE) {
			...
			//retval = aducm360_write_single(bank, buffer, offset, count);
			retval =
			    aducm360_write_modified(bank, buffer, offset,
						    count);
			if (retval != ERROR_OK) {
					   ...
					   unsigned int timeout_ms)
{
	...
	while (1) {
		target_read_u32(target,
				ADUCM360_FLASH_BASE + ADUCM360_FLASH_FEESTA,
				&v);
		if ((v & 0x00000001) == 0)
			break;
		alive_sleep(1);
