...
#define BITM_SCTLR_EL3_C	(1uL << BITP_SCTLR_EL3_C)
...
static int adspsc59x_quad_io_enable(struct flash_bank *bank, const bool enable)
{
	...
	{
		/** TODO: CCES-24915 - This relies on fixed Quad IO enable/disable commands. This should be dynamically
		 *  probed in the probe function and set via a list of known commands for each chip.
		*/
...
static int adspsc59x_erase(struct flash_bank *bank, unsigned int first, unsigned int last)
{
	...
	else if (adspsc59x_flash_info->probed != true)
	{
		LOG_ERROR("Cannot erase flash as target has not been probed. Please probe target first.");
		rc = ERROR_FLASH_BANK_NOT_PROBED;
	}
	// TODO: CCES-24914 - Check if sector is protected, see todo in protect function for more details
	/* All good to proceed */
	else
	{
		...
		for (unsigned counter = first; counter <= last; counter++)
		{
			...
			*/
			...
			// TODO: CCES-24916 - Erase command dynamically pulled from predefined commands in "flash_devices[]"
			// but will it always be a 4-byte address version? How to determine if it is?
			struct adsp_spi_flash_cmd flash_cmd = {
				.device			= (bank->driver == &adspsc59x_flash) ? ADSP_SPI_DEVICE_SC59X : ADSP_SPI_DEVICE_SC59X_A55,
		...
		while (count - buffer_index)
		{
			...
			while (count - buffer_index && chunk_count < chunk)
			{
				...
				*/
				...
				/* If the remaining bytes is less than the flash page size,
...
static int adspsc59x_get_info(struct flash_bank *bank, struct command_invocation *cmd)
{
	...
	if (adspsc59x_flash_info->probed == false)
	{
