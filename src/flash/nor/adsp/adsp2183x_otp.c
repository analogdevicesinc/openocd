	...
	uint32_t offset, uint32_t count)
{
	...
	if (target->state != TARGET_HALTED) {
		...
		uint32_t temp_buf[byte_count_rounded / 4];
		uint32_t send_buf[byte_count_rounded / 4];

		for (i = 0;	i < byte_count_rounded / 4; i++) {
			memcpy(temp_buf, buffer, BYTE_COUNT);
			buffer += BYTE_COUNT;
			converted_hex = (uint32_t)strtoul((void *)temp_buf, NULL, 16);
			send_buf[i] = converted_hex;
		}
		...
		algo_params.length = byte_count_rounded;
...
FLASH_BANK_COMMAND_HANDLER(adsp2183x_otp_bank_command)
{
	...
	do {
		...
		if (temp_str[0] != '\n' && temp_str[0] != '\r') {
			...
			adsp2183x_otp_info->adsp2183x_algorithm.adsp2183x_algo[byte_count] = converted_hex;
			byte_count++;
		}
	...
	adsp2183x_otp_info->sectorsize = temp_parse;
