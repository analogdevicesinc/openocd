...
static int aducm302x_probe(struct flash_bank *bank)
{
	...
	obj_ptr = Jim_GetGlobalVariableStr(global_cmd_ctx->interp, "_CHIPNAME", JIM_NONE);
	if (!obj_ptr) {
		LOG_ERROR("%s: _CHIPNAME is not defined", target_name(target));
		return ERROR_FAIL;
	}
	chipname = Jim_GetString(obj_ptr, NULL);

	obj_ptr = Jim_GetGlobalVariableStr(global_cmd_ctx->interp, "_CHIPID", JIM_NONE);
	if (!obj_ptr) {
		LOG_ERROR("%s: _CHIPID is not defined", target_name(target));
		return ERROR_FAIL;
	}
	retval = Jim_GetLong(global_cmd_ctx->interp, obj_ptr, &value);
	if (retval == JIM_OK)
