...
proc canfd_config { canfd_base chip_name } {
	...
	if {[string match *598* $chip_name]} {
		# use axi-ap
		set ap_num  2
		set ap_bus_access $chip_name.axi
	}
	if {[string match *594* $chip_name]} {
		# use ahb-ap
		set ap_num 0
	...
	for {set i 0} {$i < 256} {incr i} {
		mem_ap_write_reg [expr {$canfd_ram + ($i * 4)}] 0x0 $ap_bus_access
	}
	...
	for {set i 0} {$i < 64} {incr i} {
		mem_ap_write_reg [expr {$canfd_rx_imsk0 + ($i * 4)}] 0x0 $ap_bus_access
	}
	...
	for {set i 0} {$i < (168/4)} {incr i} {
		mem_ap_write_reg [expr {$canfd_ram_fd_mode + ($i * 4)}] 0x0 $ap_bus_access
	}
	...
	for {set i 0} {$i < (48/4)} {incr i} {
		mem_ap_write_reg [expr {$canfd_ram_norm_mode + ($i * 4)}] 0x0 $ap_bus_access
	}
	...
	for {set i 0} {$i < (32/4)} {incr i} {
		mem_ap_write_reg [expr {$canfd_ram_rxfir + ($i * 4)}] 0x0 $ap_bus_access
	}
...
proc adspsc5xx_init_ddr3 { dmc chip_name } {
	if { $dmc == 0 } {
	...
	if {[string match *589* $chip_name]} {
		smpu_config 0
		smpu_config 8
	} elseif {[string match *573* $chip_name]} {
		smpu_config 0
	}
	...
	} elseif {[string match *598* $chip_name]} {
...
proc adspsc59x_init_ddr3 { dmc chip_name } {
	set dmc_baseaddr       0x31070000
	set dmc_ctl            [expr {$dmc_baseaddr + 0x4}]
	set dmc_stat           [expr {$dmc_baseaddr + 0x8}]
	set dmc_cfg            [expr {$dmc_baseaddr + 0x40}]
	...
	# Enable PLL
	# pDevice->pCguRegs->CGU_PLLCTL |= BITM_CGU_PLLCTL_PLLEN
	memap_mmw $cgu0_pllctl 0x8 0 $ap_bus_access
	# Wait till PLL is enabled
	# while(!(pDevice->pCguRegs->CGU_STAT & BITM_CGU_STAT_PLLEN)) {};
	set data [mem_ap_read_reg $cgu0_stat $ap_bus_access]
	while { ![expr {$data & 0x1}] } {
		set data [mem_ap_read_reg $cgu0_stat $ap_bus_access]
	}

	# pDevice->pCguRegs->CGU_PLLCTL |= BITM_CGU_PLLCTL_PLLBPST
	memap_mmw $cgu0_pllctl 0x1 0 $ap_bus_access
	...
	mem_ap_write_reg $cgu0_ctl 0x25000 $ap_bus_access
	...
	# pDevice->pCguRegs->CGU_PLLCTL |= BITM_CGU_PLLCTL_PLLBPCL;
	memap_mmw $cgu0_pllctl 0x2 0 $ap_bus_access
